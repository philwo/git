...
--collision-check::
--no-collision-check::
	Enable or disable SHA-1 collision detection for the received
	objects, passed on to 'git index-pack' or 'git unpack-objects'.
	Without either option, the `hash.<url>.collisionDetection`
	configuration is resolved against <repository>. See
