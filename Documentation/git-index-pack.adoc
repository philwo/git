...
--collision-check::
--no-collision-check::
	Enable or disable SHA-1 collision detection while hashing the
	objects in the pack, overriding the `hash.collisionDetection`
	configuration variable. See linkgit:git-config[1] for the
	security implications. `git fetch` passes this option based on
	the `hash.<url>.collisionDetection` configuration for the
